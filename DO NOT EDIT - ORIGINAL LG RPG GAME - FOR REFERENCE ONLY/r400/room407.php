...
{	if ($equipMount == 'wooden boat')
    ...
    updateStats($link, $username,['endfight' => 0, 'room' => '408' ]); // -- update stats
        ...
        elseif ($_SESSION['flying'] >= '1') {
            ...
            $message="<i>You fly northwest! </i><br>".$_SESSION['desc408'];
            include('update_feed.php'); // --- update feed
			// $results = $link->query("UPDATE $user SET room = '408'"); // -- room change
    updateStats($link, $username,['endfight' => 0, 'room' => '408' ]); // -- update stats
        } 
...
{	if ($equipMount == 'wooden boat')
    ...
    updateStats($link, $username,['endfight' => 0, 'room' => '410' ]); // -- update stats

		}  
        elseif ($_SESSION['flying'] >= '1') {
            echo 'You fly north!<br>';
            $message="<i>You fly north! </i><br>".$_SESSION['desc410'];
            include('update_feed.php'); // --- update feed
			// $results = $link->query("UPDATE $user SET room = '410'"); // -- room change
    updateStats($link, $username,['endfight' => 0, 'room' => '410' ]); // -- update stats
        } 
...
{	if ($equipMount == 'wooden boat')
    ...
    updateStats($link, $username,['endfight' => 0, 'room' => '406' ]); // -- update stats
        ...
        elseif ($_SESSION['flying'] >= '1') {
            ...
            $message="<i>You fly south! </i><br>".$_SESSION['desc406'];
            include('update_feed.php'); // --- update feed
			// $results = $link->query("UPDATE $user SET room = '406'"); // -- room change
    updateStats($link, $username,['endfight' => 0, 'room' => '406' ]); // -- update stats
        } 
