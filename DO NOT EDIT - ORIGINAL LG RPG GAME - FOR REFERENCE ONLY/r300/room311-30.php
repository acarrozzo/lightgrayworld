...
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['desc311-30'];
...
if ($infight < 1 && $endfight != 1) {
    ...
    if (($rand <= 10) && $infight==0 && $endfight==0) {
        ...
        updateStats($link, $username,['enemy' => 'Minotaur' ]); // -- set enemy
        include('battle.php');
    } 
    /*
    ...
    */
    ...
    elseif ($input=='u' || $input=='up') {
        ...
        $message="<i class=''>You travel up the mine</i><br>".$_SESSION['desc311-29'];
        include('update_feed.php'); // --- update feed
                                // $results = $link->query("UPDATE $user SET room = 'm29'"); // -- room change
                                //  $results = $link->query("UPDATE $user SET endfight = 0"); // -- reset fight
                                $updates = ['endfight' => 0, 'room' => '311-29' ]; // -- set changes
        ...
        if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
            ...
            $message="<i class=''>You dig down to mine level 31...EVENTUALLY</i><br>".$_SESSION['desc311-30'];
            include('update_feed.php'); // --- update feed
                                        // $results = $link->query("UPDATE $user SET room = 'm30'"); // -- room change
                                        //  $results = $link->query("UPDATE $user SET endfight = 0"); // -- reset fight
                                        $updates = ['endfight' => 0, 'room' => '311-30' ]; // -- set changes
                                        updateStats($link, $username, $updates); // -- apply changes
            include('function-mine.php');	// MINE FOR ORE
        }
    }
