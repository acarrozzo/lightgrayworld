...
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['desc311-01'];
...
{			echo 'You travel up the mine<br>';
			$message="<i class=''>You travel up the mine</i><br>".$_SESSION['desc311'];
			include ('update_feed.php'); // --- update feed
   			// $results = $link->query("UPDATE $user SET room = '311'"); // -- room change
			$updates = ['endfight' => 0, 'room' => '311' ]; // -- set changes
...
{		
		if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
			...
			echo 'You dig down to mine level 2.<br>';
   			$message="<i class=''>You dig down to mine level 2.</i><br>".$_SESSION['desc311-02'];
			include ('update_feed.php'); // --- update feed
