...
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['desc311-14'];
...
{			echo 'You travel up the mine<br>';
   	$message="<i class=''>You travel up the mine</i><br>".$_SESSION['desc311-13'];
	include ('update_feed.php'); // --- update feed
   								// $results = $link->query("UPDATE $user SET room = 'm13'"); // -- room change
    $updates = ['endfight' => 0, 'room' => '311-13' ]; // -- set changes
...
{		
		if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
			...
			echo 'You dig down to mine level 15.<br>';
   			$message="<i class=''>You dig down to mine level 15.</i><br>".$_SESSION['desc311-15'];
			include ('update_feed.php'); // --- update feed
   										// $results = $link->query("UPDATE $user SET room = 'm15'"); // -- room change
    $updates = ['endfight' => 0, 'room' => '311-15' ]; // -- set changes
    updateStats($link, $username, $updates); // -- apply changes
			include ('function-mine.php');	// MINE FOR ORE			
		}
}
