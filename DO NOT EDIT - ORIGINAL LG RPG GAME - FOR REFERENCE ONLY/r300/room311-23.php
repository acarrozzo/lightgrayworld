...
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['desc311-23'];
...
{			echo 'You travel up the mine<br>';
   	$message="<i class=''>You travel up the mine</i><br>".$_SESSION['desc311-22'];
	include ('update_feed.php'); // --- update feed
   								// $results = $link->query("UPDATE $user SET room = 'm22'"); // -- room change
    $updates = ['endfight' => 0, 'room' => '311-22' ]; // -- set changes
...
{		
		if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
			...
			echo 'You dig down to mine level 23.<br>';
   			$message="<i class=''>You dig down to mine level 24 eventually.</i><br>".$_SESSION['desc311-24'];
			include ('update_feed.php'); // --- update feed
   										// $results = $link->query("UPDATE $user SET room = 'm24'"); // -- room change
    $updates = ['endfight' => 0, 'room' => '311-24' ]; // -- set changes
    updateStats($link, $username, $updates); // -- apply changes
			include ('function-mine.php');	// MINE FOR ORE			
		}
}
