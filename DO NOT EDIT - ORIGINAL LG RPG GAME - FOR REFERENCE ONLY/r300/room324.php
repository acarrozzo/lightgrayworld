...
if ($input == 'search')
{
	...
	if ($rand == 1) {
		...
		if ($rand2 ==1){
			...
			updateStats($link, $username,['bandithood' => $bandithood + 1 ]); // -- update stat 
		}
		if ($rand2 ==2){
			$rand3 = rand(5,15);
		...
		if ($rand2 ==3){
			...
			updateStats($link, $username,['meatball' => $meatball + 2 ]); // -- update stat
		}
		if ($rand2 ==4){
...
// -------------------------------------------------------------------------- TRAVEL
elseif ($input == 'north') {    $roomNum = '326';handleTravel($_SESSION['username'], $link, 'north', $roomNum, 'desc'.$roomNum.'');}
elseif ($input == 'east') {     $roomNum = '323';handleTravel($_SESSION['username'], $link, 'east', $roomNum, 'desc'.$roomNum.'');}
elseif ($input == 'south') {    $roomNum = '325';handleTravel($_SESSION['username'], $link, 'south', $roomNum, 'desc'.$roomNum.'');}
