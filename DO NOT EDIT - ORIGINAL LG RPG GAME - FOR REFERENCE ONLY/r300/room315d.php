...
// -------------------------------------------------------------------------- INITIALIZE mongolian death worm - 7/10
if(($rand <= 7 ) && $infight==0 && $endfight==0) {	
    // $results = $link->query("UPDATE $user SET enemy = 'Mongolian Death Worm'");
	updateStats($link, $username,['enemy' => 'Mongolian Death Worm' ]); // -- set enemy
	include ('battle.php'); 
}						
// -------------------------------------------------------------------------- IN BATTLE		
...
// -------------------------------------------------------------------------- TRAVEL
elseif ($input == 'east') {     $roomNum = '315a';handleTravel($_SESSION['username'], $link, 'east', $roomNum, 'desc'.$roomNum.'');}
elseif ($input == 'southwest') { $roomNum = '315c';handleTravel($_SESSION['username'], $link, 'southwest', $roomNum, 'desc'.$roomNum.'');}
