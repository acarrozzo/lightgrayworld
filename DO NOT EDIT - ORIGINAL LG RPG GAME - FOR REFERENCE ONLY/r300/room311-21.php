...
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['desc311-21'];
...
{			echo 'You travel up the mine<br>';
   	$message="<i class=''>You travel up the mine</i><br>".$_SESSION['desc311-20'];
	include ('update_feed.php'); // --- update feed
   								// $results = $link->query("UPDATE $user SET room = 'm20'"); // -- room change
    $updates = ['endfight' => 0, 'room' => '311-20' ]; // -- set changes
...
{		
		if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
			...
			echo 'You dig down to mine level 22.<br>';
   			$message="<i class=''>You dig down to mine level 22.</i><br>".$_SESSION['desc311-22'];
			include ('update_feed.php'); // --- update feed
