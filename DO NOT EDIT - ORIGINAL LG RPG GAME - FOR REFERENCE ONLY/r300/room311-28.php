...
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['desc311-28'];
...
$row = getUserData($link, $_SESSION['username']); // --- gets all user data from database
    ...
    elseif ($input=='u' || $input=='up') {
        ...
        $message="<i class=''>You travel up the mine</i><br>".$_SESSION['desc311-27'];
        include('update_feed.php'); // --- update feed
                                // $results = $link->query("UPDATE $user SET room = 'm27'"); // -- room change
                                //  $results = $link->query("UPDATE $user SET endfight = 0"); // -- reset fight
                                $updates = ['endfight' => 0, 'room' => '311-27' ]; // -- set changes
        ...
        if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
            ...
            $message="<i class=''>You dig down to mine level 29.</i><br>".$_SESSION['desc311-29'];
            include('update_feed.php'); // --- update feed
