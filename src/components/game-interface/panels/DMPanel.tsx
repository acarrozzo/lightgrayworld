...
function ThreadAvatar({ thread }: { thread: DMThread }) {
  ...
  return (
    <div className="flex h-11 w-8 items-center justify-center rounded border border-gray-700/60 bg-gray-800/60">
      {avatar ? (
...
export default function DMPanel({ onClose, onMessageSent }: DMPanelProps) {
  ...
  return (
    <div className="relative h-full w-full">
      <button
        onClick={onClose}
        className="absolute right-4 top-4 z-10 rounded-lg p-2 text-gray-400 transition-colors duration-200 hover:bg-gray-800/50 hover:text-white"
        title="Close"
        aria-label="Close"
      >
        <X size={20} />
      ...
      <div className="flex h-full flex-col gap-3 p-4 sm:p-5">
        <div className="pr-10">
          ...
          <p className="mt-1 text-xs text-gray-400">Global private messaging with offline delivery.</p>
        </div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 lg:grid-cols-[280px_1fr]">
          <div className="min-h-0 overflow-y-auto rounded-lg border border-gray-700/60 bg-gray-900/60">
            {isLoadingThreads ? (
              <div className="px-3 py-3 text-xs text-gray-400">Loading conversations...</div>
            ) : sortedThreads.length === 0 ? (
              <div className="px-3 py-3 text-xs text-gray-500">No direct message threads yet.</div>
            ) : (
                  ...
                  return (
                    <button
                      ...
                      className={`w-full px-3 py-2 text-left transition-colors ${
                        isActive ? 'bg-violet-900/30' : 'hover:bg-gray-800/60'
                      }`}
                    >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {thread.unreadCount > 0 && (
                              <span className="rounded-full bg-violet-500/90 px-1.5 py-0.5 text-[10px] font-semibold text-white">
                                {thread.unreadCount}
                              </span>
                            )}
                          </div>
                          <div className="truncate text-xs text-gray-400">{thread.lastMessageSnippet}</div>
                        </div>
                      </div>
                    </button>
                  )
          ...
          <div className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-gray-700/60 bg-gray-900/40">
            {!selectedThread ? (
              <div className="p-4 text-sm text-gray-400">Select a thread to view message history.</div>
            ) : (
              <>
                <div className="border-b border-gray-700/60 px-3 py-2 text-sm font-medium text-gray-200">
                ...
                <div className="min-h-0 flex-1 overflow-y-auto space-y-2 px-3 py-3">
                  {isLoadingMessages ? (
                    ...
                    <div className="text-xs text-gray-500">No messages yet. Say hello.</div>
                  ) : (
                            ...
                            <div
                              className={`rounded-lg px-3 py-2 text-sm ${
                                isSelf
                                  ...
                                  : 'border border-gray-700/70 bg-gray-800/70 text-gray-100'
                              }`}
                            >
                ...
                <div className="border-t border-gray-700/60 p-2">
                  ...
                  >
                    <textarea
                      ...
                      className="min-h-[56px] flex-1 resize-y rounded border border-gray-700 bg-gray-950/80 px-3 py-2 text-sm text-gray-100 outline-none transition-colors focus:border-violet-500"
                    />
                    <button
                      type="submit"
                      disabled={isSending || !composer.trim()}
                      className="inline-flex h-10 items-center gap-1 rounded bg-violet-600 px-3 text-sm font-medium text-white transition-colors hover:bg-violet-500 disabled:cursor-not-allowed disabled:opacity-60"
                    >
