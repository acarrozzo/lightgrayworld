...
export default function TabContainer({
  ...
  const getButtonColorClasses = (tab: TabConfig, isActive: boolean) => {
    ...
    if (isActive) {
      switch (color) {
        case 'blue':
          return 'border-2 border-indigo-500 hover:border-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300'
        case 'green':
          return 'border-2 border-green-500 hover:border-green-400 bg-green-500/10 hover:bg-green-500/20 text-green-300'
        case 'purple':
          return 'border-2 border-purple-500 hover:border-purple-400 bg-purple-500/10 hover:bg-purple-500/20 text-purple-300'
        case 'gold':
          return 'border-2 border-amber-500 hover:border-amber-400 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300'
        case 'red':
          return 'border-2 border-red-500 hover:border-red-400 bg-red-500/10 hover:bg-red-500/20 text-red-300'
        default:
          return 'border-2 border-indigo-500 hover:border-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300'
      }
    } else {
      return 'border-2 border-gray-600 hover:border-gray-500 bg-transparent hover:bg-gray-800/30 text-gray-400 hover:text-gray-300'
    }
  }
  ...
  return (
    <div className={`flex-1 flex flex-col min-h-0 ${containerClassName}`}>
      {/* Tab Navigation */}
      <div className={`flex gap-2 p-2 bg-gray-900/95 backdrop-blur-sm flex-shrink-0 flex-wrap items-center ${headerClassName}`}>
        {tabs.map((tab) => {
