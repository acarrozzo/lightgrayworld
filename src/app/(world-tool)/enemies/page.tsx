...
export default async function EnemiesPage() {
  ...
  const rows: EnemyRow[] = ENEMIES.map((e, i) => {
    ...
    const drops: EnemyRow['drops'] = [
      ...firstKill.map((slug) => ({ slug, name: resolveName(slug), chance: 100, tag: 'first-kill' as const })),
      ...always.map((a) => {
        const { slug, qtyLabel } = normalizeAlways(a)
        return { slug, name: `${resolveName(slug)}${qtyLabel}`, chance: 100, tag: 'always' as const }
      }),
