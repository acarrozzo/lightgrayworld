...
- `src/lib/game-data/regen.js`, `services/regen-service.js`, `services/buff-service.js`, `src/lib/status-effects.ts`: the per-click status tick. Every temporary effect is a click countdown column on `User` registered in `buff-service` (`BUFF_FIELDS`, plus `AMOUNT_FIELDS` for Iron Skin's block and Magic Armor's absorb); `engine.applyClickTick` decrements them, lands regen (gear `metadata.regen` + tea + Regenerate) then poison in one statement, and death clears them all. Enemy poison is a special in `enemy-specials.js` applied by `battle-action-handlers.applyEnemyHit`, which also spends Magic Armor before HP. The client reads the same modules for the header status strip and item labels, so a chip says what the next click does.
