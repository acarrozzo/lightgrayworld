...
- `src/lib/game-data/regen.js`, `services/regen-service.js`, `services/buff-service.js`, `src/lib/status-effects.ts`: the per-click status tick. Every temporary effect is a click countdown column on `User` registered in `buff-service` (`BUFF_FIELDS`, plus `AMOUNT_FIELDS` for the amounts Regenerate and Iron Skin lock at cast and Magic Armor's absorb; Iron Skin's amount is DEF via `getStatBuffBonuses`); `engine.applyClickTick` decrements them, lands regen (gear `metadata.regen` + tea + Regenerate) then poison in one statement, and death clears them all. Enemy poison is a special in `enemy-specials.js` applied by `battle-action-handlers.applyEnemyHit`, which also spends Magic Armor before HP. The client reads the same modules for the header status strip and item labels, so a chip says what the next click does.
...
- `src/lib/game-data/travelers.js`, `src/lib/game-engine/traveler-state.js`, `traveler-action-handlers.js`: the NPCs and creatures that move between rooms (a modern addition; the original had none). The registry says who they are and how they move; a `route` traveler's room is a pure function of UTC time, a `wander` traveler's is process-local and reset on restart. Presence is room-scoped and shared; a fightable traveler is still fought per player through the battle state and never sits in the per-player present-enemy slot. Their actions are dispatched by presence, after the room's own table.
